package com.dqty.yuxiaoer.common.utils;
...
public class RestMethodUtil {
	private static final Logger logger = Logger.getLogger(RestMethodUtil.class);
	...
	private static String postMethod(URL url, Map<String, String> parameter, Map<String, String> header) throws Exception {
		...
		if (parameter != null) {
			...
			for (Entry<String, String> entry : parameterSet) {
				String parameterKey = entry.getKey();
				String parameterValue = entry.getValue();
				submitDos.writeBytes(parameterKey + "=" + URLEncoder.encode(parameterValue, DEFAULT_CHARACTERENCODING) + "&");// 加入提交参数
			}
			submitDos.flush();
			submitDos.close();
		}
